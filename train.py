...
X = tf.placeholder(tf.float32, [BATCH_SIZE, IMG_H, IMG_W, IMG_C], name = "X")
Y = tf.placeholder(tf.float32, [BATCH_SIZE, IMG_H, IMG_W, NUM_CLASSES])
...
_pred_flatten = utils.label_before_cal_loss(_pred)
Y_flatten = utils.label_before_cal_loss(Y)
...
loss = utils.weighted_loss(logits = _pred_flatten, labels = Y_flatten)#tf.reduce_mean(softmax)
acc = utils.accuracy(logits = _pred_flatten, labels = Y_flatten)
print(loss.shape)
print("1")
